...
from products import products # Импортируем данные о продуктах
...
products_keyboard = InlineKeyboardMarkup(
    inline_keyboard=[
        [InlineKeyboardButton(text=product_data["name"], callback_data="product_buying") ] # Используем названия продуктов
...
async def send_calories(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer('Расчёт завершён. Если хотите повторить нажмите "Расчитать"', reply_markup=keyboard)
