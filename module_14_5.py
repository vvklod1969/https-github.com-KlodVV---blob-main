...
import asyncio
from crud_functions import initiate_db, get_all_products, add_user, is_included # Импортируем функции для работы с бд
import sqlite3

# Initialize the database before Dispatcher
initiate_db()
...
async def get_buying_list(message: Message):
    ...
    for product in products:
        ...
        await message.answer(f"Описание: {product[2]}")
...
async def set_age(message: Message, state: FSMContext):
     try:
        ...
        await state.set_state(None)
         ...
         await state.update_data(age=None)
         await state.set_state(RegistrationState.email)
         await message.answer(f"{e}. Введите другой email:")
